...
const SERVICE_ACCOUNT_EMAIL = process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL as string;
...
function safeGet(row: GoogleSpreadsheetRow, key: string): string {
  ...
  const value = typeof maybeGetter === 'function' ? maybeGetter.call(row, key) : (row as unknown as Record<string, unknown>)[key];
...
async function getDoc(): Promise<GoogleSpreadsheet> {
  ...
  if (!SHEET_ID || !SERVICE_ACCOUNT_EMAIL || !SERVICE_ACCOUNT_PRIVATE_KEY) {
    throw new Error('Missing Google Sheets credentials. Please set GOOGLE_SHEET_ID, GOOGLE_SERVICE_ACCOUNT_EMAIL, GOOGLE_PRIVATE_KEY');
  }
  const auth = new JWT({
...
async function ensureSheet(title: string, headerValues: string[]): Promise<GoogleSpreadsheetWorksheet> {
...
async function getLeaveRequestsSheet(): Promise<GoogleSpreadsheetWorksheet> {
  return ensureSheet(LEAVE_REQUESTS_SHEET_TITLE, LEAVE_REQUESTS_HEADERS as string[]);
}
...
export async function readLeaveHistoryByEmail(email: string): Promise<LeaveRequestRow[]> {
  const all = await readLeaveRequests();
  return all.filter(r => (r.employee_email || '').toLowerCase() === email.toLowerCase());
}

export async function addLeaveRequest(partial: Omit<LeaveRequestRow, 'id' | 'status' | 'created_at'> & { status?: string }): Promise<LeaveRequestRow> {
  const sheet = await getLeaveRequestsSheet();
...
export async function addEmployee(partial: Omit<EmployeeRow, 'id'>): Promise<EmployeeRow> {
  const sheet = await getEmployeesSheet();
